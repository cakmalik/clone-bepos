...
class CashProofInController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($proofs)
                ...
                ->addColumn('preview_url', function ($item) {
                    return route('cashProofIn.print', $item->cashProof->id);
                })
                ->make(true);
        }
        return view('pages.cash_proof.in.index');
    }

    public function create()
    {
        $cashMasters = CashMaster::all();
        return view('pages.cash_proof.in.create', compact('cashMasters'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            $proof = CashProof::create([
                ...
                'code' => autoCode('cash_proofs', 'code', 'BKM-1-' . date('y') . '-' . date('m') . '-', 4),
                'type' => 'KAS-MASUK',
            ]);
            ...
            DB::commit();
            return redirect()->route('cashProofIn.print', [$proof->id]);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function print(Request $request, $id)
    {
        ...
        return view('pages.cash_proof.in.print', compact('proof'));
    }
    ...
    public function receipt(Request $request, $id)
    {
        ...
        return view('pages.cash_proof.in.receipt', compact('proof', 'total'));
    }
}
