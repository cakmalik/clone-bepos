            ...
            <tbody class="report-content">
                <tr>
                    <td class="report-content-cell">
                        <div class="main">
                            <h3 class="report-title">Laporan Pemesanan Pembelian</h3>
                            <br><br>
                            @if ($summary_detail)
                                @forelse($orders as $order)
                                    <div>
                                        ...
                                        <table class="table">
                                            ...
                                            <tbody>
                                                @foreach ($order['purchase_detail_po'] as $item)
                                                    <tr>
                                                        <td>{{ $item['product_name'] }}</td>
                                                        <td class="text-center">{{ formatDecimal($item['qty']) }}</td>
                                        ...
                                        <thead>
                                            <tr>
                                                ...
                                                @foreach ($methods as $method => $total)
                                                    <th class="text-capitalize">PEMB. {{ $method }}</th>
                                                @endforeach
                                            </tr>
                                        </thead>
                                        <tbody>
                                            @forelse($orders as $order)
                                                <tr>
                                                    ...
                                                    <td>{{ $order['supplier']['name'] }}</td>
                                                    <td class="text-right">
                                                        {{ currency($order['purchase_invoice']['nominal']) }}</td>
                                                    <td class="text-right">
                                                        {{ currency($order['purchase_invoice']['nominal_discount']) }}
                                                    </td>
                                                    <td class="text-right">
                                                    ...
                                                    @foreach ($methods as $methodName => $total)
                                                        @php
                                                            // Ambil ID metode dari nama (kebalikan dari PAYMENT_METHODS)
                                                            $methodId = array_search($methodName, $PAYMENT_METHODS);
                                                        @endphp
                                                        <td class="text-right">
                                                            {{ currency($order['purchase_invoice']['paids'][$methodId] ?? 0) }}
                                                        </td>
                                                    @endforeach
                                            ...
                                            @empty
                                                <tr>
                                                    <td colspan="{{ 7 + sizeof($methods) }}" class="text-center">
                                                        Data Tidak Ditemukan
                                                    </td>
                                                </tr>
                                            @endforelse
                                            <tr style="border-top: 1px solid #000;">
                                                ...
                                                <td style="text-align: right; font-weight: 800;">
                                                    {{ rupiah($totalNominalPo) }}
                                                </td>
                                                <td style="text-align: right; font-weight: 800;">
                                                    {{ rupiah($totalNominalDiscount) }}
                                                </td>
                                                <td style="text-align: right; font-weight: 800;">
                                                    {{ rupiah($totalInvoice) }}
                                                </td>
